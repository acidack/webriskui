...
WEBRISK_API_SCOPES = ["https://www.googleapis.com/auth/cloud-platform"]
WEBRISK_API_ENDPOINT = "https://webrisk.googleapis.com/v1"
DATABASE = 'submissions.db'
POSSIBLE_THREAT_TYPES = [
...
POSSIBLE_ABUSE_TYPES = ["", "SOCIAL_ENGINEERING", "MALWARE", "UNWANTED_SOFTWARE", "ABUSIVE"]
POSSIBLE_PLATFORMS = ["", "ANY_PLATFORM", "PLATFORM_UNSPECIFIED", "WINDOWS", "LINUX", "ANDROID", "OSX", "IOS", "CHROME"]
POSSIBLE_CONFIDENCE_LEVELS = ["", "HIGH", "MEDIUM", "LOW", "CONFIDENCE_UNSPECIFIED"]
POSSIBLE_JUSTIFICATION_LABELS = ["USER_REPORT", "MANUAL_VERIFICATION", "AUTOMATED_DETECTION"]
...
def get_sa_credentials_from_info(key_info_dict):
    ...
    try:
        return service_account.Credentials.from_service_account_info(key_info_dict, scopes=WEBRISK_API_SCOPES), key_info_dict.get("client_email")
...
def index():
    ...
    return render_template('index.html',
        threat_types=POSSIBLE_THREAT_TYPES, abuse_types=POSSIBLE_ABUSE_TYPES,
        platforms=POSSIBLE_PLATFORMS, confidence_levels=POSSIBLE_CONFIDENCE_LEVELS,
        justification_labels=POSSIBLE_JUSTIFICATION_LABELS,
        submissions=get_all_submissions(), scan_history=session.get('scan_history', []),
...
def handle_submission():
    api_response_text, api_status_code, error_message, operation_name = "", None, None, None
    try:
        user_project_id = request.form.get('user_project_id')
        uri_to_submit = request.form.get('uri_submit')
        submission_threat_types = request.form.getlist('submission_threat_type')
        if not all([user_project_id, uri_to_submit, submission_threat_types]):
        ...
        submit_url = f"{WEBRISK_API_ENDPOINT}/projects/{user_project_id}/uris:submit"
        payload = {"submission": {"uri": uri_to_submit, "threatTypes": submission_threat_types}}
        response = authed_session.post(submit_url, headers={'Content-Type': 'application/json'}, json=payload)
        api_status_code, api_response_text = response.status_code, response.text
        response.raise_for_status()
        operation_name = response.json().get('name')
        if operation_name:
            log_submission(user_project_id, uri_to_submit, submission_threat_types, operation_name)
            flash(f"Submission successful! Operation: {operation_name}", "success")
        else:
            flash("Submission successful, but no operation name was returned.", "warning")
    except (ValueError, BadRequest, json.JSONDecodeError) as e: error_message = f"Configuration Error: {e}"
    except requests.exceptions.RequestException as e:
        try: error_detail = json.loads(api_response_text).get('error', {}).get('message', api_response_text)
        except (json.JSONDecodeError, AttributeError): error_detail = api_response_text
        error_message = f"API Request Failed (Status {api_status_code}): {error_detail}"
    ...
    session['api_status'] = api_status_code; session['error_message'] = error_message
    session['operation_name'] = operation_name
    return redirect(url_for('index'))

@app.route('/check_status', methods=['POST'])
def handle_check_status():
    api_response_text, api_status_code, error_message = "", None, None
...
def handle_evaluate():
    ...
    try:
        ...
        headers = { 'Content-Type': 'application/json', 'User-Agent': 'Flask-Web-Risk-UI/3.2' }
    ...
    session['last_action'] = 'evaluate'; session['api_response'] = api_response_text
    session['api_status'] = api_status_code; session['error_message'] = error_message
    return redirect(url_for('index'))
